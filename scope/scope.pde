...
int i, j = 0;                 //counters
int values[] = new int [101]; //array that stores the probe values 
...
void display_values() {
  ...
  stroke(238, 255, 5);
  fill(238, 255, 5);
  values[100] = values[99]; //ugly way of handling the last data point
  for (i = 0; i < values.length - 1; i++) {
    line(x_start + x_len_data * i, y_end - values[i] / 2, x_start + x_len_data * (i+1), y_end - values[i +1] / 2); //display the yellow line
    val_low = values[i] < val_low ? values[i] : val_low; //check if the value is lower than the previous lowest value and sets it as the new lowest value
    val_high = values[i] > val_high ? values[i] : val_high; //check if the value is higher than the previous highest value and sets it as the new highest value
    total += values[i]; //ad to the total value
  }
  strokeWeight(1); //set the style for the display values
  textAlign(LEFT, TOP);
  fill(51);
  rect(x_start, y_end, 150, 20);       //clear the spots to display the peak-peak and average values
  ...
  text("peak-peak :" + str(float(pk_pk)/100) + "V", x_start, y_end); //display the values
...
void draw() {
  ...
  while (myPort.available() > 4)  //check if at least 4 bytes are available
  {
    ...
    if (input != null)  //double check if the input is not empty
    {
      ...
      default:
        if (data.length > 1) {
          values[int(data[0])] = int(data[1]);//store the values in the data array
        }
        break;
      }
    }
